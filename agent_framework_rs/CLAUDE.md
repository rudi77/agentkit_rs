...
**Strategy has two layers.** `Strategy::{React, Plan, Plain}` is only a system-prompt preamble — no separate execution path, faithful to the Python original. On top of it, `src/strategy.rs` adds `RunStrategy` (a documented deviation): `Direct(Strategy)` is exactly the old behaviour, `PlanExecute` is a *driver around* `run_on_bus` (plan phase → one ReAct run per step → optional reflection with bounded rework). The driver never touches `Agent::drive`; its phase runs carry a non-empty event `source` ("plan", "schritt 1/3", …) and it emits the single root FINAL/DONE itself — frontends stop on the first empty-`source` DONE, so keep that contract. Failure sentinels pass through verbatim (swarm/work depend on them).

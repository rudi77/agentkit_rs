...
- **Ablehnung geht an den Urheber zurück** (`MessageKind::Critique`, budgetfrei). Eine Stimme geht sonst ausschließlich an den CompletionActor — der Vorschlagende erführe also nie, DASS und WARUM er abgelehnt wurde, und eine Protokollregel „arbeite die Einwände ein" wäre eine Anweisung zu etwas Unmöglichem. Jedes Mitglied merkt sich dafür die Urheber der Vorschläge, die es gesehen hat (`SwarmToolContext::gesehene_vorschlaege`); ist der Urheber kein direkter Nachbar, meldet das Tool-Ergebnis unter `einwand_an_urheber` ehrlich `nicht_zugestellt`. Erst damit ist die Ablehnung eine Runde und keine Sackgasse — der bis dahin gemessene Effekt war, dass Mitglieder pauschal zustimmten, weil ein Nein den Schwarm nur ins Limit laufen ließ.

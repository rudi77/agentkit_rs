...
| `swarm_vote` | Über einen Vorschlag abstimmen (geht **nur** an den CompletionActor — **außer** bei Ablehnung, siehe unten) |
...
- **Abstimmungsfrist** (`vote_window`, Default 20 s): Das Erreichen des Quorums beendet den Schwarm NICHT sofort. Danach werden weiter Stimmen gesammelt, bis entweder alle außer dem Vorschlagenden geantwortet haben oder die Frist abläuft; dann gewinnt der **bestunterstützte** Vorschlag, nicht der schnellste. Ohne die Frist entschied die allererste Stimme: in einem echten Lauf mit drei Mitgliedern (Quorum 1) trafen danach noch zwei Vorschläge und drei Stimmen ein, darunter sämtliche des Mitglieds, das am meisten gearbeitet hatte. `Duration::ZERO` stellt das alte Verhalten wieder her.
- **Ablehnung geht an den Urheber zurück** (`MessageKind::Critique`, budgetfrei). Eine Stimme geht sonst ausschließlich an den CompletionActor — der Vorschlagende erführe also nie, DASS und WARUM er abgelehnt wurde, und eine Protokollregel „arbeite die Einwände ein" wäre eine Anweisung zu etwas Unmöglichem. Jedes Mitglied merkt sich dafür die Urheber der Vorschläge, die es gesehen hat (`SwarmToolContext::gesehene_vorschlaege`); das Tool-Ergebnis meldet unter `einwand_an_urheber` unterscheidbar `zugestellt`, `kein_nachbar` (Urheber außerhalb der Topologie), `urheber_unbekannt` oder `entfällt` (bei Zustimmung gibt es nichts zuzustellen). In einem echten Lauf ging so eine von zwei Ablehnungen verloren — sichtbar nur, weil der Status den Unterschied benennt. Erst damit ist die Ablehnung eine Runde und keine Sackgasse — der bis dahin gemessene Effekt war, dass Mitglieder pauschal zustimmten, weil ein Nein den Schwarm nur ins Limit laufen ließ.
...
| `ergebnis_schema` | Gestalt des erwarteten GESAMTergebnisses (JSON oder Text). Steht im Prompt JEDES Mitglieds, das einen Abschluss-Vorschlag damit vollständig ausfüllen muss. Dringend empfohlen bei mehreren Perspektiven: ohne Schema kennt ein Mitglied nur seinen eigenen Teil und schlägt genau den als Abschluss vor — beobachtet als zwei konkurrierende Teilvorschläge statt einer Synthese |

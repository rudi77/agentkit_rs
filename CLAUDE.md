...
| `agentkit_graph/` | **agentkit-graph** — graph-based knowledge on top of agentkit: a Working Graph for the running task, a Canonical Graph for durable knowledge, mandatory provenance, `graph_*` tools and a `GraphAgent` recall wrapper. Storage is an in-memory snapshot (`RwLock<Arc<GraphIndex>>`) plus an OKF v0.2 bundle on disk (Markdown documents with YAML frontmatter, one per entity) — **no storage dependency, no C compiler**. Not a port; design decisions in its `README.md`. |
| `agentkit_work/` | **agentkit-work** — persistent runtime for work that outlives a single agent run: `WorkProject`/`WorkRun`/`WorkItem` with dependencies, leases, attempts, artifacts, retry and budgets, driven by a synchronous single-worker loop that runs ordinary agentkit agents. Storage is in-memory `RwLock<Arc<WorkState>>` plus an append-only JSONL journal whose compaction *is* the checkpoint; **no storage dependency, no C compiler**. (This used to be "the same pattern as agentkit_graph"; it no longer is — agentkit_graph moved to an OKF bundle, work keeps the journal because a work log is a chronology, not a knowledge corpus.) Owns the `agentkit work …` CLI logic. Not a port; design decisions and deviations from `docs/plans/agent-work-runtime.md` in its `README.md`. |

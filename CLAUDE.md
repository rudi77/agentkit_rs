...
| `agentkit_viz/` | **agentkit-viz** — the observation tool: reads the NDJSON trace a run writes (`agentkit --trace DIR`, see `agentkit::trace`) and serves agents/history/context/timeline/work as JSON plus one embedded HTML page. Hexagonal: `model`/`project` are the domain (no HTTP, no filesystem), `trace` and `server` are the adapters. Read-only, binds only to `127.0.0.1`, requires a start-time random token. Its `tiny_http` dependency (synchronous, no TLS) is the only new foreign crate in the repo and stays behind the `viz` feature. Owns the `agentkit viz …` CLI logic. Design decisions and the deliberate deviation from `docs/plans/agentkit-viz-plan.md` (polling instead of SSE) are in its `README.md`. |
